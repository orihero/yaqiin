...
router.get('/dashboard', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        let analytics = yield Analytics_1.default.findOne({}, {}, { sort: { date: -1 } });
        let stats;
        if (analytics) {
            ...
            // Orders & Revenue
            const [totalOrders, totalOrdersLastWeek, totalOrdersLastMonth, totalOrdersLastYear, totalRevenue, totalRevenueLastWeek, totalRevenueLastMonth, totalRevenueLastYear, totalUsers, totalUsersLastWeek, totalUsersLastMonth, totalUsersLastYear, newUsers, newUsersLastWeek, newUsersLastMonth, newUsersLastYear, totalProducts, totalShops] = yield Promise.all([
                ...
                Order.aggregate([{ $group: { _id: null, total: { $sum: '$pricing.total' } } }]).then((r) => { var _a; return ((_a = r[0]) === null || _a === void 0 ? void 0 : _a.total) || 0; }),
                Order.aggregate([{ $match: { createdAt: { $gte: startOfLastWeek, $lt: startOfWeek } } }, { $group: { _id: null, total: { $sum: '$pricing.total' } } }]).then((r) => { var _a; return ((_a = r[0]) === null || _a === void 0 ? void 0 : _a.total) || 0; }),
                Order.aggregate([{ $match: { createdAt: { $gte: startOfLastMonth, $lt: startOfMonth } } }, { $group: { _id: null, total: { $sum: '$pricing.total' } } }]).then((r) => { var _a; return ((_a = r[0]) === null || _a === void 0 ? void 0 : _a.total) || 0; }),
                Order.aggregate([{ $match: { createdAt: { $gte: startOfLastYear, $lt: startOfYear } } }, { $group: { _id: null, total: { $sum: '$pricing.total' } } }]).then((r) => { var _a; return ((_a = r[0]) === null || _a === void 0 ? void 0 : _a.total) || 0; }),
                User.countDocuments({}),
            ...
            const deliveryTimes = yield Order.aggregate([
            ...
            const deliveryMinutesArr = deliveryTimes.map((d) => d.deliveryMinutes).filter((x) => typeof x === 'number');
            const averageDeliveryTime = deliveryMinutesArr.length ? (deliveryMinutesArr.reduce((a, b) => a + b, 0) / deliveryMinutesArr.length) : null;
            ...
            const topProducts = yield Order.aggregate([
                { $unwind: '$items' },
                { $group: {
                        _id: '$items.productId',
            ...
            const usersTimeSeries = yield User.aggregate([
            ...
            const paymentsTimeSeries = yield Order.aggregate([
                { $match: { createdAt: { $gte: new Date(now.getFullYear(), now.getMonth(), now.getDate() - 6) } } },
                { $group: {
                        _id: {
