"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.sendOrderStatusUpdateToGroup = sendOrderStatusUpdateToGroup;
exports.createStatusActionButtons = createStatusActionButtons;
const telegram_1 = require("../../utils/telegram");
/**
 * Sends order status updates to the orders group with action buttons for admins/operators
 */
function sendOrderStatusUpdateToGroup(telegram, order, shop, status, updatedBy) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!shop || !shop.orders_chat_id)
            return;
        const User = require('../../models/User').default;
        const client = yield User.findById(order.customerId);
        const clientName = client ? `${client.firstName || ''} ${client.lastName || ''}`.trim() : 'Client';
        const clientPhone = (client === null || client === void 0 ? void 0 : client.phoneNumber) || '';
        const statusEmojis = {
            created: '📝',
            operator_confirmed: '✅',
            packing: '📦',
            packed: '📦',
            courier_picked: '🚚',
            delivered: '🎯',
            paid: '💵',
            rejected_by_shop: '❌',
            rejected_by_courier: '❌',
            cancelled_by_client: '🚫',
        };
        const statusLabels = {
            created: 'Buyurtma yaratildi',
            operator_confirmed: 'Buyurtma tasdiqlandi',
            packing: 'Buyurtma tayyorlanmoqda',
            packed: 'Buyurtma tayyor',
            courier_picked: 'Kuryer buyurtmani oldi',
            delivered: 'Buyurtma yetkazildi',
            paid: "Buyurtma to'landi",
            rejected_by_shop: 'Do\'kon tomonidan rad etildi',
            rejected_by_courier: 'Kuryer tomonidan rad etildi',
            cancelled_by_client: 'Mijoz tomonidan bekor qilindi',
        };
        const emoji = statusEmojis[status] || '📋';
        const statusLabel = statusLabels[status] || status;
        let orderText = `${(0, telegram_1.escapeMarkdownV2)(`${emoji} ${statusLabel}`)}\n` +
            `Order ID: ${(0, telegram_1.escapeMarkdownV2)(String(order._id))}\n` +
            `Mijoz: ${(0, telegram_1.escapeMarkdownV2)(clientName)}`;
        if (clientPhone) {
            const safePhone = clientPhone.replace(/[-()\s]/g, '');
            orderText += `\nTelefon: [${(0, telegram_1.escapeMarkdownV2)('📞 ' + clientPhone)}](tel:${(0, telegram_1.escapeMarkdownV2Url)('+' + safePhone)})`;
        }
        orderText += `\n\n${(0, telegram_1.escapeMarkdownV2)('Mahsulotlar:')}`;
        for (const item of order.items) {
            const line = `- ${item.name} x${item.quantity} (${item.price} x ${item.quantity} = ${item.subtotal})`;
            orderText += `\n${(0, telegram_1.escapeMarkdownV2)(line)}`;
        }
        orderText += `\n\n${(0, telegram_1.escapeMarkdownV2)('Umumiy:')} ${(0, telegram_1.escapeMarkdownV2)(String(order.pricing.total))}`;
        if (updatedBy) {
            const updaterName = updatedBy.firstName || updatedBy.username || 'Unknown';
            orderText += `\n\n${(0, telegram_1.escapeMarkdownV2)('Yangilagan:')} ${(0, telegram_1.escapeMarkdownV2)(updaterName)}`;
        }
        const buttons = createStatusActionButtons(order, status);
        yield telegram.sendMessage(shop.orders_chat_id, orderText, {
            parse_mode: 'MarkdownV2',
            reply_markup: buttons ? { inline_keyboard: buttons } : undefined,
        });
    });
}
function createStatusActionButtons(order, currentStatus) {
    const buttons = [];
    const statusTransitions = {
        created: ['operator_confirmed', 'rejected_by_shop'],
        operator_confirmed: ['packing', 'rejected_by_shop'],
        packing: ['packed', 'rejected_by_shop'],
        packed: ['courier_picked', 'rejected_by_shop'],
        courier_picked: ['delivered', 'rejected_by_courier'],
        delivered: ['paid'],
        paid: [],
        rejected_by_shop: [],
        rejected_by_courier: [],
        cancelled_by_client: [],
    };
    const statusLabels = {
        created: '📝 Yaratildi',
        operator_confirmed: '✅ Tasdiqlash',
        packing: '📦 Tayyorlash',
        packed: '📦 Tayyor',
        courier_picked: '🚚 Kuryer oldi',
        delivered: '🎯 Yetkazildi',
        paid: "💵 To'landi",
        rejected_by_shop: '❌ Do\'kon rad etdi',
        rejected_by_courier: '❌ Kuryer rad etdi',
        cancelled_by_client: '🚫 Mijoz bekor qildi',
    };
    const statusEmojis = {
        created: '📝',
        operator_confirmed: '✅',
        packing: '📦',
        packed: '📦',
        courier_picked: '🚚',
        delivered: '🎯',
        paid: '💵',
        rejected_by_shop: '❌',
        rejected_by_courier: '❌',
        cancelled_by_client: '🚫',
    };
    const next = statusTransitions[currentStatus] || [];
    if (next.length > 0) {
        const row = [];
        next.forEach((st) => {
            row.push({ text: `${statusEmojis[st]} ${statusLabels[st]}`, callback_data: `admin_status_${st}_${order._id}` });
        });
        buttons.push(row);
    }
    return buttons;
}
