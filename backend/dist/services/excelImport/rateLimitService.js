...
class RateLimitService {
    ...
    getModelRateLimits(model) {
        ...
        if (modelLower.includes('openai/'))
            provider = 'openai';
        else if (modelLower.includes('google/'))
            provider = 'google';
        else if (modelLower.includes('meta-llama/'))
            provider = 'meta';
        else if (modelLower.includes('mistralai/'))
            provider = 'mistral';
        else if (modelLower.includes('deepseek/'))
            provider = 'deepseek';
        else if (modelLower.includes('moonshotai/'))
            provider = 'moonshot';
        else if (modelLower.includes('tencent/'))
            provider = 'tencent';
        else if (modelLower.includes('tngtech/'))
            provider = 'tngtech';
        else if (modelLower.includes('shisa-ai/'))
            provider = 'shisa';
        else if (modelLower.includes('z-ai/'))
            provider = 'z-ai';
    ...
    initializeApiKeys() {
        ...
        for (let i = 1; i <= 16; i++) {
            ...
            const baseUrl = process.env[`OPENROUTER_BASE_URL_${i}`] ||
            ...
            if (key) {
                ...
                const capabilities = [];
                if (!rateLimits.supportsSystemPrompt)
                    capabilities.push('no-system');
                if (!rateLimits.supportsJsonMode)
                    capabilities.push('no-json');
                const capabilityStr = capabilities.length > 0 ? ` [${capabilities.join(', ')}]` : '';
                console.log(`🔑 Found API key ${i + 1}: ${model} (${rateLimits.isFreeModel ? "FREE" : "PAID"}, ${rateLimits.requestsPerMinute}/min)${capabilityStr}`);
            }
        }
        console.log(`🚀 Initialized ${keys.length} API keys for rate limit handling`);
        // Log all available keys for debugging
        keys.forEach((keyConfig, index) => {
            const capabilities = [];
            if (!keyConfig.supportsSystemPrompt)
                capabilities.push('no-system');
            if (!keyConfig.supportsJsonMode)
                capabilities.push('no-json');
            const capabilityStr = capabilities.length > 0 ? ` [${capabilities.join(', ')}]` : '';
            console.log(`   Key ${index + 1}: ${keyConfig.model} (${keyConfig.provider}, ${keyConfig.isFreeModel ? "FREE" : "PAID"}, ${keyConfig.requestsPerMinute}/min)${capabilityStr}`);
        });
        return keys;
    }
    getCurrentKey() {
    ...
    getOptimalThreadCount() {
        ...
        const totalRequestsPerMinute = this.apiKeys.reduce((sum, key) => sum + key.requestsPerMinute, 0);
        ...
        console.log(`🧵 Calculated optimal thread count: ${optimalThreads} (based on ${totalRequestsPerMinute} total requests/min across ${this.apiKeys.length} keys)`);
        console.log(`   Free models: ${freeModelKeys.length}, Paid models: ${paidModelKeys.length}`);
     ...
     */
    getApiKeyConfigs() {
        return this.apiKeys.map((key) => (Object.assign(Object.assign({}, key), { key: "***" }))); // Hide actual keys for security
    }
    /**
     * Get key distribution statistics for a given number of threads
     */
    getKeyDistributionStats(threadCount) {
        ...
        const isEvenlyDistributed = Math.max(...threadCounts) - Math.min(...threadCounts) <= 1;
        return {
            totalKeys,
    ...
    getRateLimitStatus() {
        ...
        return this.apiKeys.map((keyConfig, index) => {
            ...
            const isAtLimit = keyConfig.currentRequests >= keyConfig.requestsPerMinute;
            return {
                keyIndex: index,
     ...
     */
    assignKeyToThread(threadNumber) {
        if (this.apiKeys.length === 0) {
        ...
        console.log(`🔑 Thread ${threadNumber} assigned to Key ${keyIndex + 1} (${keyConfig.model}) - Total keys available: ${this.apiKeys.length}`);
        return keyIndex;
    }
    /**
     * Get the key assigned to a specific thread
     */
    getKeyForThread(threadNumber) {
    ...
    waitForRateLimitReset(keyConfig, keyIndex) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (keyConfig.currentRequests >= keyConfig.requestsPerMinute) {
                ...
                console.log(`⏳ Rate limit reached for key ${keyIndex + 1} (${keyConfig.model}). Waiting ${Math.ceil(waitTime / 1000)}s...`);
    ...
    rotateToNextKey() {
        ...
        if (this.currentKeyIndex === 0 && this.apiKeys.length > 1) {
            console.log(`🔄 Rotated through all ${this.apiKeys.length} API keys, back to key 1`);
        }
        else {
            console.log(`🔄 Switched from key ${previousIndex + 1} to key ${this.currentKeyIndex + 1}`);
        }
    }
    makeRequest(messages_1) {
        return __awaiter(this, arguments, void 0, function* (messages, options = {}, productName = "Unknown", threadNumber) {
            var _a, _b, _c;
            ...
            while (attempts < maxAttempts) {
                ...
                const keyConfig = threadNumber !== undefined
                    ? this.getKeyForThread(threadNumber)
                    : this.getCurrentKey();
                const keyIndex = threadNumber !== undefined
                    ? this.threadKeyAssignments.get(threadNumber)
                    : this.currentKeyIndex;
                const threadPrefix = threadNumber !== undefined ? `Thread ${threadNumber} - ` : "";
                try {
                    // Wait for rate limit reset if needed
                    yield this.waitForRateLimitReset(keyConfig, keyIndex);
                    // Create OpenAI client for this key
                    const openai = new openai_1.default({
                        apiKey: keyConfig.key,
                        baseURL: keyConfig.baseUrl,
                    });
                    console.log(`🤖 ${threadPrefix}Using key ${keyIndex + 1} (${keyConfig.model}) for product: ${productName}`);
                    // Prepare request options based on model capabilities
                    const requestOptions = Object.assign({ model: keyConfig.model }, options);
                    ...
                    if (!keyConfig.supportsJsonMode && ((_a = requestOptions.response_format) === null || _a === void 0 ? void 0 : _a.type) === 'json_object') {
                    ...
                    const metrics = {
                        ...
                        duration,
                        success: true,
                        tokensUsed: (_b = response.usage) === null || _b === void 0 ? void 0 : _b.total_tokens,
                    };
                    this.performanceLog.push(metrics);
                    console.log(`✅ ${threadPrefix}Product "${productName}" processed in ${duration}ms using ${keyConfig.model} (key ${keyIndex + 1})`);
                    ...
                    if (((_c = error.message) === null || _c === void 0 ? void 0 : _c.includes("rate limit")) || error.status === 429) {
                        ...
                        if (threadNumber !== undefined) {
                            ...
                            console.log(`⏳ Thread ${threadNumber} waiting ${backoffDelay / 1000}s for rate limit reset...`);
            ...
            throw new Error(`All ${this.apiKeys.length} API keys failed after ${attempts} attempts`);
        });
    }
    getPerformanceReport() {
        const totalRequests = this.performanceLog.length;
        const successfulRequests = this.performanceLog.filter((m) => m.success).length;
        const failedRequests = totalRequests - successfulRequests;
        const totalTime = this.performanceLog.reduce((sum, m) => sum + m.duration, 0);
        ...
        this.apiKeys.forEach((keyConfig, index) => {
            byKey[`Key ${index + 1}`] = {
                ...
                averageTime: keyConfig.totalRequests > 0
                    ? keyConfig.totalTime / keyConfig.totalRequests
                    : 0,
                errors: keyConfig.errors,
                rateLimitHits: keyConfig.rateLimitHits,
                successRate: keyConfig.totalRequests > 0
    ...
    printPerformanceReport() {
        ...
        Object.entries(report.byThread).forEach(([threadName, data]) => {
        ...
        // Find fastest model
        const fastestModel = Object.entries(report.byModel).reduce((fastest, [model, data]) => {
            return data.averageTime < fastest.averageTime
                ? Object.assign({ model }, data) : fastest;
        }, { model: "", averageTime: Infinity });
        if (fastestModel.model) {
            console.log(`\n🏆 Fastest Model: ${fastestModel.model} (${Math.round(fastestModel.averageTime)}ms average)`);
        }
        console.log("📊 ================================\n");
    }
    /**
    ...
    printRateLimitStatus() {
        ...
        status.forEach((keyStatus) => {
            ...
            console.log(`${statusIcon} Key ${keyStatus.keyIndex + 1} (${keyStatus.model} [${modelType}]): ${keyStatus.currentRequests}/${keyStatus.requestsPerMinute} requests${resetInfo}`);
        });
        console.log("🚦 ==============================\n");
    }
}
